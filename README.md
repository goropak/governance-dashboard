...
> 2026-06-10 기준 구축된 보좌관↔시장 무인 소통 루프.
...
### 3. 텔레그램으로 push 승인하는 법 (단계별)

> 시장(Claude Code)이 커밋을 마치면 push-pending.md에 신호를 기록합니다.
> 이후 흐름은 다음과 같습니다.

**순서**:
1. 핸드폰 텔레그램에 알림 도착 → "push 대기" 메시지 + ✅/❌ 버튼
2. 메시지 내용 확인 (커밋 요약 + 변경 파일 통계 포함)
3. **✅ 승인** 버튼 클릭 → 게이트가 자동으로 `git push` 실행
4. "✅ 승인 + push 완료" 메시지로 성공 확인
5. (또는 ❌ 거부 버튼 → push 취소, 대통령이 직접 검토)

**전제조건**: telegram-gate가 켜져 있어야 합니다 (아래 섹션 참조).

---

### 4. telegram-gate 켜고 끄는 법
...
**자동 push 흐름 (v0.2 구현됨)**:
- ✅ 버튼 클릭 → 게이트가 즉시 `git push` 자동 실행.
- push 성공 시 "✅ 승인 + push 완료" 메시지 수신.
- push 실패 시 에러 메시지 수신 → 대통령이 직접 재시도 가능.
