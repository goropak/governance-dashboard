...
4. `../governance/active/outbox-to-mayor.md`에서 **이 도시(governance-dashboard) 앞으로 온 지시**를 찾는다.
5. 그 지시를 처리한다. (이 도시 지시가 없으면 "대기 지시 없음"으로 보고하고 멈춘다 — 다른 도시 지시는 건드리지 않는다.)
6. 규칙·교훈이 필요하면 `../governance/`를 **읽기로만** 참조. 쓰기·커밋은 이 레포 안에서만.
7. 끝나면 `STATUS.md` 갱신 + 결과를 `../governance/active/inbox-from-mayor.md`에 한 줄 기록. push는 승인 게이트(`approvals/`) 경유 — 임의 push 금지.
